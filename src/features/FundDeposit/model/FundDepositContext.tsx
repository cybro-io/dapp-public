...
import { getChain, triggerToast } from '@/shared/lib';
import {
  useErc20Balance,
  useTokenPriceUsd,
  useAppKitAccount,
...
const useFundDepositValues = () => {
  ...
  /* Calculate onRamp */
  React.useEffect(() => {
    if (isSelectedDefaultToken || selectedToken.isCrypto || !fiatConvert) {
      return;
    }
  ...
  const isNeedSwitchNetwork =
    selectedToken.isCrypto && selectedToken.chain.id !== chainId;
