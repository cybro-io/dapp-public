...
import { $symbiosis, getChain, isErrorUserRejected } from '@/shared/lib';
...
const swapFx = createEffect<SwapCalculateResult, void, void>(
  async (calculate) => {
    ...
    await appKit.switchNetwork(getChain(transactionRequest.chainId));
...
swapFx.fail.watch(({ error, params }) => {
  console.error(error);

  const { from, tokenAmountIn, tokenAmountOut, transactionRequest } = params;

  track.event(
    isErrorUserRejected(error)
      ? AnalyticsEvent.RejectSwap
      : AnalyticsEvent.ErrorSwap,
    {
      walletAddress: from,
      fromAmount: Number(tokenAmountIn.toSignificant()),
      fromCurrency: tokenAmountIn.token.name,
      contractAddress:
        'to' in transactionRequest ? transactionRequest.to : null,
      toAmount: Number(tokenAmountOut.toSignificant()),
      toCurrency: tokenAmountOut.token.name,
