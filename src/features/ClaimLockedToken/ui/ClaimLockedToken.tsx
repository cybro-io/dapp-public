...
import {
  useClaimedCybroBalance,
  useCybroBalance,
  useStakingConfig,
} from '@/entities/Staking';
import { ConnectWallet } from '@/features/ConnectWallet';
import { links, triggerToast, useWeb3ModalAccount } from '@/shared/lib';
import {
  Button,
  Group,
  Link,
...
import { StakingBalance } from '@/widgets/Staking/ui/StakingBalance';
...
export const ClaimLockedToken = () => {
  const { address } = useWeb3ModalAccount();

  const { config } = useStakingConfig();
  const { balance, isLoadingBalance } = useCybroBalance(address);
  const { claim, isLoadingClaimedBalance } = useClaimedCybroBalance(address);
  ...
  const handleClaim = () => {
    ...
    claimLockedTokenMutation(config.token.locked)
      .then(() => {
        triggerToast({
