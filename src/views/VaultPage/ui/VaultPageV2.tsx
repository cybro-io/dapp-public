...
import { getExplorerProvider, links, useAppKitAccount } from '@/shared/lib';
import {
  AssetIcon,
...
export const VaultPageV2 = ({ vaultId }: VaultPageV2Props) => {
  const { vault, tokens, fees, isLoading, vaultTvl, cybroTvl, vaultParams } =
    useVaultPage(vaultId);

  const distributionKey = useMemo(() => {
    return vault?.tags?.find((value) => ['seasonal', 'index'].includes(value));
  }, [vault]);
      ...
      <div className="flex flex-row justify-center md:justify-between items-center flex-wrap gap-2">
        <Group className="items-center gap-4 justify-center md:justify-start text-center md:text-left">
          ...
          <Stack className="gap-2">
            ...
            {vault.badges && (
              <Group className="gap-1.5 justify-center md:justify-start">
