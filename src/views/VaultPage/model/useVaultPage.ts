...
import {
  ...
  useGetSeasonalFundInfoApiV1VaultsSeasonalFundIdGet,
} from '@/shared/types';
...
export const useVaultPage = (vaultId: number) => {
  ...
  const { data: seasonalVaultData, isLoading: isLoadingSeasonalVaultData } =
    useGetSeasonalFundInfoApiV1VaultsSeasonalFundIdGet(vaultId, {
      query: {
        queryKey: [QueryKey.SeasonalVaultChart],
        enabled: vault?.tags?.includes('seasonal'),
      },
    });

  const vaultSeason = seasonalVaultData?.data.data?.season as
