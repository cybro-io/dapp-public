...
import { useBalanceErc20Query, useToast, useAppKitAccount } from '@/shared/lib';
...
export const useExchange = () => {
  ...
  const { data: cybroBalance, isLoading: isLoadingCybroBalance } =
    useBalanceErc20Query({
      token: cybroContractAddress.cybro,
      address,
      chainId: ChainId.BLS,
    });
  ...
  const { data: tokenBalance, isLoading: isLoadingTokenBalance } =
    useBalanceErc20Query({
      token: form.paymentMethod.isCrypto
