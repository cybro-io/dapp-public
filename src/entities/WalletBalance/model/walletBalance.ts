...
import { ChainId, isBtcChainId, isTronChainId, Token } from 'symbiosis-js-sdk';

import { $symbiosis, getEthTokensBalanceByChain } from '@/shared/lib';
...
const getWalletBalancesFunc = async ({
  ...
  try {
    ...
    const chains = symbiosis
      .chains()
      .filter((chain) => !isBtcChainId(chain.id) && !isTronChainId(chain.id));
