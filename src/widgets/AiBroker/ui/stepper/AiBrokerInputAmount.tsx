...
export const AiBrokerInputAmount = () => {
  ...
  const handleAcceptAmount = () => {
    if (isDisabledSubmit || !selectedFund) {
      return;
    }
    ...
    addChatItem([
      {
        ...
        position: 'right',
        content: `${formatUserMoney(amount)} ${selectedToken.symbol} ≈ $${formatUserMoney(getTokenAmountUsd(amount, price), 2)}`,
      },
      {
        id: nanoid(),
        ...
        position: 'left',
        amount,
        amountUsd: getTokenAmountUsd(amount, price),
        token: selectedToken,
        fund: selectedFund,
      },
    ]);
  ...
  if (lastMessage?.type !== AiBrokerMessageType.selectTokenAndAmount) {
    return null;
  }
