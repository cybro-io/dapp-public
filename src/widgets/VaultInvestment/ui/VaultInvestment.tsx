...
import { periodTabs } from '@/widgets/BalanceHistory';
...
export const VaultInvestment = ({
  ...
  const { period, setPeriod, vaults } = useMyVaults({
    ...
    // todo: will made filter by vaultId
    limit: 100,
  });

  const [vaultToken, setVaultToken] = React.useState('');
  const vault = vaults.find((vault) => vault.fund.id === vaultId);

  useEffect(() => {
    if (vaultAddress && vault) {
      const vaultContract = createTokenContract(
        vaultAddress,
        getProviderByChainId(vault.fund.chain_id)!,
      );
        ...
        <Group className="justify-between gap-3">
          <Field title="Current Balance" className="flex-[2.7]">
            <Group className="md:flex-nowrap items-center gap-2">
              <AssetIcon
                src={vault.token.icon ?? links.noImage}
          ...
          <Group className="justify-between md:flex-nowrap flex-[1.8] gap-3">
            <Field
              title={
                <InfoButtonTooltip
                  content={`1 ${vaultToken} ≈ ${formatUserMoney(sharePrice, 6, 6)} ${vault.token.name}`}
                >
                  <Typography
                    order={4}
                    variant="caption"
